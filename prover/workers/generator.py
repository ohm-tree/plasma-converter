...
from prover.utils import MODEL_FORMAT, AttrDict
...
class GeneratorProcess(mp.Process):
    ...
    def run(self):
        seed = int(time.time()) % 1000 + \
            (self.node_rank * 8 + self.local_rank) * 1000
        os.environ['LOCAL_RANK'] = str(self.local_rank)
        llm = LLM(model=self.model_path, max_num_batched_tokens=8192, seed=seed,
                  trust_remote_code=True, dtype=torch.float16, tensor_parallel_size=4)
        while True:
            ...
            outputs = [self.output_func(_output.outputs[0].text)
                       for _output in model_outputs]
            with self.lock:
