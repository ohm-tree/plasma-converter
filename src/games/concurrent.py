...
class Router(Generic[T]):
    ...
    def enqueue_task(self, msg: WorkerTask, source: T):
        ...
        if task_type in self.active_counts:
            self.active_counts[task_type] += 1
        else:
            self.active_counts[task_type] = 1
        self.total_active += 1
    ...
    def dequeue_tasks(self, blocking=False, timeout=None) -> Iterator[Tuple[WorkerResponse, T]]:
        ...
        for response in responses:
            ...
            if response.task_id.task_type in self.active_counts:
                self.active_counts[response.task_id.task_type] -= 1
            else:
                raise ValueError(
                    f"Task type {response.task_id.task_type} not in active counts.")
            self.total_active -= 1
