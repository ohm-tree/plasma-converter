...
from src.games.lean_game_core import LeanMove, LeanState, LeanStateError
...
class MetaLeanState(ConcurrentMetaGameState[LeanState, MetaLeanMove]):
    def __init__(self,
                 worker_id: WorkerIdentifer,
                 internal_state: LeanState,
                 comment: str,
        ...
        # so pointers to the MetaLeanState children are stored here.
        self.children = {}
    ...
    @classmethod
    def saves(cls, states: List['MetaLeanState'], filename: str):
        """
    ...
    @classmethod
    def loads(cls, filename: str) -> List['MetaLeanState']:
        """
    ...
    def starting_state(cls,
                       ...
                       max_depth: int = 20) -> 'MetaLeanState':

        internal_state = LeanState.starting_state(
            worker_id=worker_id,
            problem=problem,
            header=header,
    ...
    def next_state(self, action: MetaLeanMove) -> LeanState:
        """
        ...
        self.children[action] = MetaLeanState(
    ...
    @require_ready
    def get_active_move(self, index: int) -> MetaLeanMove:
        return self.gen_comments[index]

    @require_ready
    def active_moves(self) -> List[MetaLeanMove]:
        return self.gen_comments

    @require_ready
    def index_active_move(self, move: MetaLeanMove) -> int:
    ...
    def require_new_move(self) -> None:
        raise NotImplementedError(
            "This function is not implemented for LeanState")
    ...
    def post_comments(self, results: WorkerResponse) -> Iterator[WorkerTask]:
        ...
        self.gen_comments = [
            MetaLeanMove(comment) for comment in results.response['comments']
        ]
        ...
        for i, comment in enumerate(self.gen_comments):
            if comment in seen:
                self.gen_comments[i] = MetaLeanMove(
