...
def collate_results(run: str):
    ...
    num_solved = {'valid': 0, 'test': 0}
    denominator = {'valid': 0, 'test': 0}
    for result in all_results:
        if result['result'] == '1.0':
            num_solved[result['split']] += 1
        denominator[result['split']] += 1

    # save the results in a file
    with open(f'results/summary_{run}.txt', 'w') as file:
        file.write(run + '\n')
        file.write(
